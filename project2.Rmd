...
business <- business %>% mutate(workweek = 
...
business <- business %>% mutate(title_words = 
            if_else(n_tokens_title > results[[5]] , 'Long', 
                    if_else(n_tokens_title > results[[3]], 'Typical', 'Short')))

results[[5]]
business$title_words <- factor(business$title_words, 
...
library(caret)
# want 70% to train on
trainIndex <- createDataPartition(business$shares, p = .7, list = FALSE)


# subsets of data
busTrain <- business[trainIndex, ]
busTest <- business[-trainIndex, ]
...
LM1 <- train(shares ~ num_imgs + num_videos, data = busTrain,
             method = "lm", 
             preProcess = c('center', 'scale'),
             trControl = trainControl(method = 'cv', number = 10))
LM1

pred1 <- predict(LM1, newdata = busTest)
postResample(pred1, obs = busTest$shares)


# Linear model based on when it was released Mon - Thur vs Fri - Sun

LM2 <- train(shares ~ workweek, data = busTrain,
             method = "lm", 
             preProcess = c('center', 'scale'),
             trControl = trainControl(method = 'cv', number = 10))
LM2

pred2 <- predict(LM2, newdata = busTest)
postResample(pred2, obs = busTest$shares)

# Linear model based on length

LM3 <- train(shares ~ I(n_tokens_content)^2 + n_tokens_title, data = busTrain,
             method = "lm", 
             preProcess = c('center', 'scale'),
             trControl = trainControl(method = 'cv', number = 10))
LM3

pred3 <- predict(LM3, newdata = busTest)
postResample(pred3, obs = busTest$shares)
